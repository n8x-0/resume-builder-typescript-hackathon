...
document.addEventListener('click', (e: Event) => {
    ...
    if (target.classList.contains('shareButton')) {
        const link = target.getAttribute("data-link") as string;
...
const handleUploadImage = async (e: Event): Promise<void> => {
...
const handleCvData = (data: CVdata): void => {
...
const resetFormFields = (form: HTMLFormElement): void => {
    const { username, contact, email, objective, summary } = form;
...
const shareableLink = (cvData: CVdata): string => {
    ...
    const link = `${window.location.origin}/src/view-resume.html?data=${encodedData}`;
    return link;
}
